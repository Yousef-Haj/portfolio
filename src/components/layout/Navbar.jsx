...
import {NAV_LINKS,PERSONAL_INFO,} from '../../utils/constans'
import { useScrollyspy, scrollToSection } from "../../hooks/useScrollSpy";
...
const Navbar = () => {
  ...
  const activeSection = useScrollyspy (NAV_LINKS.map(link => link.id));
  ...
  return (
    <nav className={`fixed top-0 right-0 z-1000 w-full py-4 transtion-all duration-300 ${isScrolled ? 'bg-black/30 backdrop-blur-lg' : 'bg-transparent' }`} style={{transform:'translate3d(0,0,0)'}}>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            <button
             ...
             className="text-2xl front-bold bg-linear-to-r from-primary , via-primary/50 to-primary/30 bg-clip-text text-transparent"
             aria-label="home"
            >
              {PERSONAL_INFO.name}
            ...
            <div className="hidden md:flex items-center gap-2">
              <button
                ...
                className="py-3.5 px-7 bg-white text-[#212121] font-medium text-base  rounded-[17px] border border-white hover:bg-white/90  transition-all duration-300"
              >
                Hire Me
              </button>
            </div>
      ...
      {/*mobile menu*/}
      <div className={`md:hidden transition-duration-300 overflow-hidden  ${isMenuOpen ? 'max-h-96 opacity-100' :'max-h-0 opacity-0' }`}>
